...
class BootLoadControl(object):
	...
	def __init__(self):
		try:
			self.verbose = False
			self.scan=posfidcan.PosFidCAN('can1')		
	...
	def request_verification(self, can_ids):
		...
		try:
			...
			pass #return 'FAILED: Error requesting bootloader verification: %s' % str(e)
...
#	os.system('sudo config-pin "P9_11" 0')  #set PS1_EN low
#	time.sleep(sleep_time)
#	os.system('sudo config-pin "P9_12" 1')  #raise the SYNC line
#	time.sleep(sleep_time)
#	os.system('sudo config-pin "P9_11" 1')  #set PS1_EN high
#	time.sleep(sleep_time)

	hex_file_name = 'bootreloader38.hex' 
	bc=BootLoadControl()
	print('Programming ' + hex_file_name + '.....')
	bc.program(brdcast_id, hex_file_name)
	bc.request_verification([brdcast_id])
	time.sleep(1)
	hex_file_name = 'bootloader38.hex'
	bc2=blc.BootLoadControl()

	os.system('cansend can1 004e2080#4d.2e.45.2e.4c.65.76.69')	
	print('Programming ' + hex_file_name + '.....')
	bc2.program(brdcast_id, hex_file_name)
	print(bc2.request_verification([brdcast_id]))

#	os.system('sudo config-pin "P9_12" 0') #set SYNC low
