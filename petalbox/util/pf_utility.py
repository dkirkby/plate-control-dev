...
if __name__ == '__main__':
	...
	while loop:
		...
		print("[l]ist all IDs (Si, FW, BL, CAN) found on the CAN bus. Needs FW 4.X if more than one device on bus")
		print("[s]id programming - type in silicon ID, then program new CAN ID")
		print("[i]ndividually address a pos/fid on bus by CAN ID and ask for its fw version and silicon ID")
		print("\nThe following settings are used to set/read motor and fiducial parameters stored in flash (FW > 4.3)")
		print("------------------------------------------------------------------------------------------------------")
