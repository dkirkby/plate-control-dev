...
pulled from online databases:
    ...
    "calib" --> http://desi-www.kpno.noao.edu:8080/DESIPositioners/Positioner_calibration.html
    "moves" --> http://desi-www.kpno.noao.edu:8080/DESIPositioners/Positioner_moves.html
    
Data is assumed saved as csv using the browser tool.

Sample SQL queries:
    
    "calib" --> select petal_id,pos_id,time_recorded,device_loc,length_r1,length_r2,offset_x,offset_y,offset_t,offset_p,physical_range_t,physical_range_p,gear_calib_t,gear_calib_p from posmovedb.positioner_calibration_p3 order by time_recorded desc
    "moves" --> select petal_id,pos_id,time_recorded,ctrl_enabled from posmovedb.positioner_moves_p3 order by time_recorded desc
...
calibdb_name = 'PTL03 online calib db, retrieved 2020-02-11.csv'
movesdb_name = 'PTL03 online moves db, retrieved 2020-02-11.csv'
