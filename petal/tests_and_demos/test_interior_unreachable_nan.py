...
p.state.write('LENGTH_R1',3.0444912734348404)
p.state.write('LENGTH_R2',3.0592099185275723)
p.state.write('OFFSET_T',33.6990237206073)
p.state.write('OFFSET_P',-4.40183469230584)
p.state.write('OFFSET_X',85.76099236364983)
p.state.write('OFFSET_Y',35.80931562807018)
p.state.write('PHYSICAL_RANGE_T',396.77360363210005)
p.state.write('PHYSICAL_RANGE_P',190.0)
print('targetable range theta: ' + str(p.targetable_range_T))
print('targetable range phi: ' + str(p.targetable_range_P))
print('|length r1 - length r2| --> ' + str((p.state.read('LENGTH_R1') - p.state.read('LENGTH_R2'))))
...
p.state.write('LENGTH_R1',2.8717353438946156)
p.state.write('LENGTH_R2',3.0843491074195599)
p.state.write('OFFSET_T',-65.309004215659343)
p.state.write('OFFSET_P',-6.5767604546238516)
p.state.write('OFFSET_X',53.797074126319295)
p.state.write('OFFSET_Y',80.715848473197767)
p.state.write('PHYSICAL_RANGE_T',395.16777127611419)
p.state.write('PHYSICAL_RANGE_P',188.81443605360451)
print('targetable range theta: ' + str(p.targetable_range_T))
print('targetable range phi: ' + str(p.targetable_range_P))
print('|length r1 - length r2| --> ' + str((p.state.read('LENGTH_R1') - p.state.read('LENGTH_R2'))))
