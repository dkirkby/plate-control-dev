...
ptl = petal.Petal(petal_id, posids, fidids, simulator_on=True, db_commit_on=False, local_commit_on=True,verbose=False, anticollision='adjust',petal_shape='asphere')
...
for i in range(len(posids)):
    posid=posids[i]
    pos_tp_arr.append([ptl.get_posfid_val(posid,'POS_T'),ptl.get_posfid_val(posid,'POS_P')])
...
def read_targets(ptl,target_file):
    ...
    for x in lines:
        if x.strip() != '-1' and x.strip() and match_target:
            ...
            if min(distance) <=ptl.get_posfid_val(posids[index],'LENGTH_R1')+ptl.get_posfid_val(posids[index],'LENGTH_R2') :
                if posids[index] not in pos_assigned: 
                    ...
                    pos_assigned.append(posids[index])
    f.close()
    ...
    for i in range(len(posids)):
        ...
        while 1:
            ...
            if posid in pos_assigned and posid not in pos_collide:
                ...
                #tp1,unreachable=postrans.QS_to_posTP(target) # posTP
                tp1,unreachable=postrans.obsXY_to_posTP([x_assigned[index],y_assigned[index]])
                if unreachable:
                   print('Target ',tp1, ' from list unreachable')
                   pos_assigned[index]='999' 
                   continue
                ...
                if unreachable:
                   print('Random target',tp1,' unreachable, try again')
                   continue
                tp=postrans.posTP_to_obsTP(tp1)
                distance=np.sqrt(xy[0]**2+xy[1]**2)
                print('random xy',xy,'distance',distance)
                if distance<=ptl.get_posfid_val(posid,'LENGTH_R1')+ptl.get_posfid_val(posid,'LENGTH_R2'):  # Make sure it is reachable
                    # Check for collision #
                    if index_neighbour:
                        for k in range(len(index_neighbour)):
                            index_this_neighbour=index_neighbour[k]
                ...
                if distance<=ptl.get_posfid_val(posid,'LENGTH_R1')+ptl.get_posfid_val(posid,'LENGTH_R2'):
