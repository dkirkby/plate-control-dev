...
fidids = fidids_12
petal_id = 666
target_file='tile541.txt'
target_file2='tile560.txt'
match_target=True
output_file='time_trial_541.pdf'
        ...
        if index_neighbour:
            for k in range(len(index_neighbour)):
                ...
                if posids[index_this_neighbour] in pos_checked:
                    print('This is checked before, skip')
                else:
...
# timed test sequence
cProfile_wrapper('ptl = petal.Petal(petal_id, posids, fidids, simulator_on=True, db_commit_on=False, local_commit_on=True,verbose=True)')
    ...
    for i in range(len(ptl.posmodels)):
        ...
        while 1:
            ...
            if posid in pos_assigned and posid not in pos_collide:
                ...
                tp1,unreachable_this=postrans.QS_to_posTP(target)
...
collision_result=check_collision(ptl,target_tp)
...
if ps:
    pp.savefig()
"""
pos_assigned,xy_assigned,x_assigned,y_assigned,qs_assigned=read_targets(ptl,target_file2)
requests,target_x,target_y,target_tp=assign_targets(ptl,pos_assigned,qs_assigned,x_assigned,y_assigned)
plt.plot(target_x,target_y,'b+',label=label1)
for i in range(len(offsetX_arr)):
    plt.text(offsetX_arr[i],offsetY_arr[i],posids[i],fontsize=2)
