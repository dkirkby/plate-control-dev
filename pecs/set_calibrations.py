...
for key in keys:
    ...
    commit_type_ok = table[commit_key].dtype in [int, bool]     # Python builtins work for dtype comparison
    assert2(commit_type_ok, f'{commit_key} data type must be boolean or integer representing boolean')
    data_type_ok = column.dtype in [int, float, bool]    # Python builtins work for dtype comparison
