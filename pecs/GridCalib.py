...
class Grid(PECS):
    '''
    Grid
    subclass of PECS that adds functions to run an Grid
    ...
    def calibration(self, auto_update=True, user_input=False, match_radius=80):
        '''
        Executes an Grid calibration and sets data and data_path attributes
        '''
        ...
        for request in req_list:
            self.printfunc(f'Measuring grid point {i} of {len(req_list)}')
            i += 1 
        ...
        data = self.ptl.calibrate_from_grid_data(grid_data, auto_update=auto_update)
    ...
    def set_data(self, filename=None, old=False):
        '''
        ...
        old=True allows for support of old obsXY files
        '''
        if old:
            coord = 'OBS'
        else:
            coord = 'PTL'
        converters = {'MEAS_'+coord+'X':literal_eval,
                      'MEAS_'+coord+'Y':literal_eval,
                      'TARGET_P':literal_eval,
                      'TARGET_T':literal_eval,
                      'FLAGS_LIST':literal_eval,
                      'ERR_NORM':literal_eval,
                      'POINT_NUMBERS':literal_eval,
                      'EXP_'+coord+'X':literal_eval,
                      'EXP_'+coord+'Y':literal_eval,
                      'OFFSET_X_LIST':literal_eval,
                      'OFFSET_Y_LIST':literal_eval,
                      'OFFSET_T_LIST':literal_eval,
                      'OFFSET_P_LIST':literal_eval,
                      'LENGTH_R1_LIST':literal_eval,
                      'LENGTH_R2_LIST':literal_eval}
    ...
    def run_interactively(self, petal=None, posids=None, auto_update=None,
        ...
        '''
        self.printfunc('Running interactive Grid Calibraiton')
...
if __name__ == '__main__':
    grid = Grid()
    grid.run_interactively(user_input=True, match_radius=80)
