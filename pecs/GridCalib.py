...
class Grid(PECS)
	...
	def grid_calibration(selection=[],n_points_P=6, n_points_T=6,auto_update=True):
		if not selection:
		...
		for request in requests_list:
			...
			meas_data.append(measured_positions)
		updates, data = self.ptls[self.ptlid].calibrate_from_grid_data(meas_data,auto_update=auto_update)
		return updates, data
...
if __name__ == '__main__':
	...
	updates, data = grid.grid_calibration(selection=selection)
	print(updates)
