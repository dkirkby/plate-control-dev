...
class PosCalibrations(PECS):
    ...
    def __init__(self, fvc=None, ptls=None, pcid=None, posids=None,
                 interactive=False):
        self.n_pts_T = 6
        self.n_pts_P = 6
        self.data = CalibrationData('arc', PECS().pcids,
    ...
    def run_arc_calibration(self, auto_update=False, match_radius=50,
        ...
        # collect old/current calibration values
        for pcid in self.pcids:  # use posids of each petal for multi-petal ver
            self.ptls[pcid].collect_calib(
                pd.DataFrame({'DEVICE_ID': self.posids}, tag=''))
