...
class PosMoveMeasure(object):
    ...
    def __init__(self, petals, fvc, printfunc=print):
        ...
        self.wide_spotmatch_radius = 80.0 # [pixels on FLI FVC CCD] wide search radius used during rough calibration when theta and phi offsets are unknown
    ...
    def one_point_calibration(self, posids='all', mode='posTP', wide_spotmatch=False, in_place=False):
        ...
        The in_place argument allows for the calibration to proceed without moving the positioners.
        """
        ...
        if mode == 'posTP' or mode == 'offsetsTP' or mode == 'offsetsTP_close':
            ...
            if mode == 'posTP' and in_place:
                data,imgfiles = self.measure()
                self._test_and_update_TP(data, mode)
            else:
    ...
    def _remove_outlier_calibration_points(self, data, mode):
        ...
        for posid in data:
            ...
            for i in range(n_pts):
                if data[posid]['measured_obsXY'][i] != [0,0]: # matched spot case
