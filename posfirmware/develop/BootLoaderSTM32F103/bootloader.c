...
const uint32_t		buffer_size									=  15360;					 	// Buffer size in words. 5120 words = 20480 Bytes = 20KB. 
...
//	Set_Up_EXTI_Regs();   											//	These are currently used only for the demonstration/test sequences
														//  Set up the Alternate GPIO Functions (Note that we are not doing any of the remapping shown on page 177)
...
//send_CANmsg(pos_id, 2, 128, 0);		//Send "ready for firmware update" message (id 128)
	...
	//send_CANmsg(pos_id, 2, command, 0);  //test
	...
	if (command == 129) {
		...
		//send_CANmsg(pos_id, 4, code_size, 0);
	}
	...
	//flash_PA6(50);									//  Flash PA6 for 50 msec when a CAN message is recognized
	command = CAN_RxMsg.id &= 0xFF;
	//send_CANmsg(pos_id, 2, command, 0);
	if (command == 130) {
		number_of_parts = 0;
		...
		//send_CANmsg(pos_id, 2, number_of_parts, 0);
	}
	else return 2;			
	...
	for (n=0; n<number_of_parts; n++){
		...
		ReadyForPartN:
		//send_CANmsg(POSITIONER_NR, 1, 131, 0);		//Send "ready for part n" message (id 131)

		for (p=0;p<code_size;p++){					// for all packets in part n
			while(!CAN_RxRdy);					    		// Wait for CAN message 130
			CAN_RxRdy = 0;
			//flash_PA6(50);										//  Flash PA6 for 50 msec when a CAN message is recognized
			//command = CAN_RxMsg.id &= 0xFF;
			//if (command != 132) return 3;
			//if (CAN_RxMsg.data[0] != n) return 4;
			//packet = 0;
			//packet |= CAN_RxMsg.data[2];
			//packet |= CAN_RxMsg.data[1] << 8;
			//if (packet != p) return 5;
			// TO DO: verify order of bytes in AppliRxBuffer
			...
			send_CANmsg(pos_id, 4, AppliRxBuffer[p], 0);
			
		} //end for buffer_size. Part n is now complete
		
		for (p=0;p<code_size;p++){ // send all packets of part n back
			
			//Txdata = 0; 													//set data[0-8] to 0
			//Txdata |= n;								  				//set data[0] to n
			//Txdata |= p << 8;											//set data[1-2] to p
			//Txdata |= AppliRxBuffer[p] << 24;     //set data[3-6] to packet p
			
			send_CANmsg(pos_id, 4, AppliRxBuffer[p], 0);		//send packet p back
			
		}	// end of send part n back	
		
		// Wait for Verification 1 result
		//while(!CAN_RxRdy);					    		// Wait for CAN message 134
		//CAN_RxRdy = 0;
		//flash_PA6(50);										//  Flash PA6 for 50 msec when a CAN message is recognized
		//command = CAN_RxMsg.id &= 0xFF;
		//if (command != 134) return 6;
		
		//nanif (CAN_RxMsg.data[0] != 1) goto ReadyForPartN;	// If verification 1 not OK, do part n again
	
		//write buffer to flash and verify number of write operations
		write_operations = WriteRxBuffer(APPLI_START_ADDR + n*4*code_size);
		send_CANmsg(POSITIONER_NR, 8, 135, (write_operations == code_size));
		//if (write_operations != buffer_size) goto EraseFlash;
		
		//read all packets from flash and send them for verification 3			
		ptr = (unsigned long *) APPLI_START_ADDR + n*4*code_size;
		
		for (p=0;p<code_size;p++){ // send all packets of part n back
		Txdata = 0; 								//set data[0-8] to 0
		Txdata |= n;						 		//set data[0] to n
		Txdata |= p << 8;					 	//set data[1-2] to p
		Txdata |= ptr[p] << 24;    	//set data[3-6] to packet p
		
		send_CANmsg(POSITIONER_NR, 8, ptr[p], 0);		//send packet p back		
		}	// end of send part n back	

	} // end for number_of_parts
	jump2Appli();
	while(!CAN_RxRdy);					    		// Wait for CAN message 130
	CAN_RxRdy = 0;
