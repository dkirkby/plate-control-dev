...
void Set_Up_Standard_GPIO(void)									//  Sets up the GPIO ports which are used as ordinary inputs or outputs:
{
	...
	*ptr |= 0x00000100;														//  (sleep) on the CAN interface IC
	...
	ptr = (unsigned long *) GPIOB_ODR;   					// Set PB2 to ODR bit to 0 to complete pull-down input configuration, set PB5 (switch enable output) to 0
	*ptr &= 0x00000000;														// Set to reset value, PB2(sync) is configured with internal pull down resistor when its ODR bit is 0, all other GPIOB pins
																								// can be set at 0 in ODR in bootload mode
	...
	*ptr = 0x44444444;														//  Set to reset value
	*ptr &= 0xFF0FF0FF;
	*ptr |= 0x00100800;
...
int main (void)
{
  ...
  while(command != 130){
	...
	for (n=0; n<number_of_parts; n++){
		...
		for (p=0; p< buffer_size; p++){				// for all packets in part n
			...
			if (currentp >= (code_size-1)) currentp = p, p = (buffer_size);							//leave loop, no more packets expected
		
			currentp += 1;
		} //end for buffer_size. Part n is now complete
		
		for (i=currentp; i< buffer_size; i++){
				AppliRxBuffer[i] = 0;
		}
		
		//write buffer to flash and verify number of write operations
		write_operations = WriteRxBuffer(APPLI_START_ADDR + n*4*buffer_size);  
