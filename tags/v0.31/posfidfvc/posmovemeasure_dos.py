...
class PosMoveMeasure(object):
    ...
    def __init__(self, petals, fvc):
        ...
        # using Pyro objects are keys is a bad idea so we are switching to a different dictionary
        self.Petals = {}
        for petal in petals:
            pid = petal.get('petal_id')
            self.Petals[pid] = petal
    ...
    def move(self, requests):
        ...
        for pid in pos_ids_by_ptl.keys():
            these_requests = {}
            for pos_id in pos_ids_by_ptl[pid]:
                these_requests[pos_id] = requests[pos_id]
            petal = self.Petals[pid]
    ...
    def rehome(self,pos_ids='all'):
        ...
        for pid in pos_ids_by_ptl.keys():
            petal = self.Petals[pid]
            print(repr(pos_ids_by_ptl[pid]))
            petal.request_homing(pos_ids_by_ptl[pid])
    ...
    def all_pos_ids(self):
        ...
        for pid in pos_ids_by_ptl.keys():
            all_pos_ids.extend(pos_ids_by_ptl[pid])
        return all_pos_ids
    ...
    def _identify(self, pos_id=None):
        ...
        for i in range(len(nudges)):
            ...
            if self.fvc.fvc_type == 'simulator':
                xy_meas = []
                for pid, petal in self.Petals.items():
                    positioners_current = petal.expected_current_position(posid=pos_ids_by_ptl[pid],key='obsXY')
