...
fvc.scale = 0.0282 # mm/pixel (update um_scale below if not in mm)
fvc.rotation = 0  # deg
um_scale = 1000 # um/mm
pos_ids = ['UM00022']
fid_can_ids = []
...
script_exec_hours = int( np.floor(script_exec_time/3600.))

print('Total test time: '+format(script_exec_time/60.,'.2f') +' minutes (' +format(script_exec_hours,'2d')+' hours '+format((script_exec_time-script_exec_hours*3600.)/60.,'.2f')+' minutes)')