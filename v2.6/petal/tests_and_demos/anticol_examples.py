...
def general_move(command,targets):
    ...
    for target in targets:
        ...
        if use_standard_syntax:
            ...
            ptl.schedule_moves(should_anneal=False)    # all the requests get scheduled, with anticollision calcs, generating a unique table of scheduled shaft rotations on theta and phi axes for every positioner 
            ...
            # ptl.schedule_send_and_execute_moves(should_anneal=False) # alternative wrapper which just does the three things above in one line
...
if should_anticol_example:
    ...
    case=2
    if case == 1:
        # Case1: Unreachable target
        requests['M00137']={'command':command, 'target':[5.5,0], 'log_note':log_note}
        requests['M00138']={'command':command, 'target':[-5.5,0], 'log_note':log_note}
    elif case == 2:
        # Case2: Type2 Collision
        requests['M00137']={'command':command, 'target':[5,-2], 'log_note':log_note}
        requests['M00138']={'command':command, 'target':[-5,1], 'log_note':log_note}
    elif case == 3:
        # Case3: Type 3 Collision
        requests['M00137']={'command':command, 'target':[5.5,0], 'log_note':log_note}
        requests['M00138']={'command':command, 'target':[-5.5,0], 'log_note':log_note}
